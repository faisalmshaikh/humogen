...
class OutlineReportController
{
    ...
    private function buildOutlineExportDocument(string $tableHtml, string $filename): string
    {
        ...
        return '<!doctype html><html lang="en"><head><meta charset="utf-8">'
            ...
            . '.outline-export-toolbar{display:flex;justify-content:flex-end;gap:.5rem;margin-bottom:.5rem}'
            . '.outline-edit-button{border:1px solid #999;background:#f8f9fa;color:#555;cursor:pointer;font-size:.9rem;line-height:1;padding:.3rem .45rem}'
            . '.outline-edit-button:hover{color:#000;background:#e9ecef}'
            . '#outline-export-status{margin-left:.75rem}'
            . '</style></head><body>'
            . '<div class="outline-export-toolbar"><button type="button" class="outline-edit-button" id="outline-export-edit" title="Edit table" aria-label="Edit table">✎</button>'
            . '<button type="button" id="outline-export-submit">Submit Changes</button><span id="outline-export-status" role="status"></span></div>'
            . $tableHtml
            . '<script>'
            ...
            . 'function enableOutlineTableEditing(){outlineExportTable.querySelectorAll("tbody tr").forEach(row=>row.querySelectorAll("td").forEach((cell,index)=>{if(outlineExportEditableColumns.includes(index)){enableOutlineCellEditing(cell);}}));}'
            . 'document.getElementById("outline-export-edit").addEventListener("click",enableOutlineTableEditing);'
            ...
            . 'cell.addEventListener("dblclick",()=>enableOutlineCellEditing(cell));'
